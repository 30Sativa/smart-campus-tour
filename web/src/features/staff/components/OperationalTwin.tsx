...
const toolClass = (on: boolean) =>
  `inline-flex min-h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9cc93a] ${on ? 'bg-[#1c1c1c] text-white' : 'bg-white/90 text-[#44474e] ring-1 ring-[#dbe8b8] hover:bg-white'}`
...
export function OperationalTwin({ robots, tour, selectedStopId, className = '' }: {
  ...
  return (
    <section className={`relative overflow-hidden rounded-2xl border border-[#dbe8b8] bg-[#efefe9] ${className}`} aria-label="Digital Twin vận hành 3D">
      <div className="absolute inset-x-0 top-14 bottom-0" role="img" aria-label={tour ? `Bản đồ 3D: ${focusRobot?.name ?? 'robot'} và các POI của ${tour.code}` : `Bản đồ 3D với ${placed.length} robot`}>
        <Suspense fallback={<div className="grid size-full place-items-center text-sm text-[#6b6e75]"><span className="flex items-center gap-2"><LoaderCircle size={16} className="animate-spin" />Đang tải bản đồ 3D…</span></div>}>
          <TwinScene robots={twinRobots} route={route} selectedStopId={selectedStopId} overhead={overhead} showLabels={labels} />
        </Suspense>
      </div>

      <div className="pointer-events-none absolute inset-x-0 top-0 flex min-h-14 flex-wrap items-center justify-between gap-2 border-b border-[#e3e3dc] bg-[#fbfbf8] p-3">
        <span className="pointer-events-auto inline-flex items-center gap-2 text-xs font-bold text-[#1c1c1c]">
          <LiveDot tone="ok" />Operational Twin
          {sources.length > 0 && <span className="font-mono text-[10px] font-normal text-[#8e9096]">· có dữ liệu {sources.join(', ')}</span>}
        </span>
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-wrap items-end justify-between gap-2 p-3">
        <span className="rounded-lg bg-white/90 px-3 py-2 text-[11px] font-semibold text-[#5c5f66] shadow-sm ring-1 ring-[#dbe8b8] backdrop-blur">
          {tour ? 'Số = thứ tự POI · ⚑ điểm kết thúc · không vẽ đường Nav2' : 'Vị trí từ telemetry, hệ tọa độ bản đồ robot'}
        </span>
        {missing > 0 && <span className="rounded-lg bg-white/90 px-3 py-2 text-[11px] font-semibold text-[#8a5a06] shadow-sm ring-1 ring-[#dbe8b8]">{missing} robot không gửi vị trí</span>}
      </div>
    </section>
  )
}
