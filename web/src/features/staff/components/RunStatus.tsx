  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {!compact && <span className="text-xs text-[#8e9096]">{done}/{total} POI · rev {tour.revision}</span>}
      </div>
      <dl className={`mt-4 grid grid-cols-2 gap-x-4 gap-y-4 ${compact ? 'rounded-xl bg-[#f7f7f3] p-4 [&_dd]:text-xs [&_dt]:text-[10px]' : ''}`}>
      ...
      <ul className={`mt-4 space-y-1.5 ${compact ? 'text-[11px]' : 'text-[13px]'}`}>
        <li className={`flex items-center gap-2 ${stale ? 'font-bold text-[#8a5a06]' : 'text-[#6b6e75]'}`}>
        ...
        <li className="flex items-center gap-2 text-[#6b6e75]">
          <Send size={14} aria-hidden="true" />
          {progress?.pendingCommand ? <span>Đang chờ kết quả: <strong className="text-[#1c1c1c]">{progress.pendingCommand.label}</strong> <span className="font-mono text-xs">{progress.pendingCommand.id}</span></span> : 'Không có lệnh đang chờ'}
        </li>
        <li className="flex items-center gap-2 text-[#6b6e75]">
