      ...
      <TableFrame label={label} wide={showTour}>
        <thead className="border-b border-[#f1f5f9] bg-[#f8fafc]">
          <tr>
            ...
            <th scope="col" className={`${thClass} sticky right-0 bg-[#f8fafc] text-right shadow-[-8px_0_12px_-10px_rgba(31,49,77,0.25)]`}>Thao tác</th>
          </tr>
        </thead>
            ...
            return (
              <tr key={reg.id} className={waiting ? 'bg-[#fffbf2] shadow-[inset_3px_0_0_#e3a83b] transition-colors hover:bg-[#fff7e8]' : rowClass}>
                <td className={`${tdClass} min-w-44`}>
                  <p className="font-semibold text-[#0f172a]">{reg.schoolName}</p>
                  <p className="text-xs text-[#94a3b8]">Gửi {formatStamp(reg.submittedAt)}</p>
                ...
                {showTour && (
                  <td className={tdClass}>
                    <p className="max-w-48 truncate font-medium text-[#1e293b]" title={reg.tourName}>{reg.tourName}</p>
                    <p className="text-xs whitespace-nowrap text-[#94a3b8]">{reg.tourCode} · {formatSlot(reg.tourScheduledAt)}</p>
                  </td>
                )}
                <td className={tdClass}>
                  <p className="whitespace-nowrap text-[#1e293b]">{reg.representativeName}</p>
                  <p className="text-xs whitespace-nowrap text-[#64748b]">{reg.contactEmail}</p>
                </td>
            ...
            <li key={reg.id} className={`p-4 ${waiting ? 'bg-[#fffbf2]' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-bold text-[#1e293b]">{reg.schoolName}</p>
                  <p className="text-xs text-[#64748b]">{reg.representativeName}</p>
                </div>
                <RegistrationStateBadge state={reg.state} />
              </div>
              {showTour && <p className="mt-2 text-xs text-[#64748b]">{reg.tourCode} · {reg.tourName}</p>}
              <p className="mt-2 flex items-center gap-1.5 text-[13px] text-[#334155]"><Users size={14} className="text-[#94a3b8]" aria-hidden="true" />{reg.studentCount} học sinh, gửi {formatStamp(reg.submittedAt)}</p>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-bold text-[#1e293b]">Danh sách học sinh <span className="font-semibold text-[#64748b]">({query ? `${rows.length}/` : ''}{roster.length})</span></p>
        <SearchField value={query} onChange={setQuery} label="Tìm học sinh" placeholder="Tìm theo họ tên, lớp…" className="w-full sm:w-64" />
      </div>
      <div className="mt-3 max-h-80 overflow-y-auto rounded-xl border border-[#e2e8f0]">
        <table className="w-full text-left text-sm" aria-label="Danh sách học sinh">
          ...
          <tbody className="divide-y divide-[#f1f5f9]">
            {rows.map((row) => (
              <tr key={row.index}>
                <td className="px-4 py-2 text-xs text-[#94a3b8] tabular-nums">{row.index}</td>
                <td className="px-4 py-2 font-semibold text-[#1e293b]">{row.name}</td>
                <td className="px-4 py-2 text-[#334155]">{row.className || <span className="text-[#94a3b8]">-</span>}</td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={3} className="px-4 py-6 text-center text-sm text-[#94a3b8]">Không có học sinh nào khớp “{query}”.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
