    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-sm leading-6 text-[#6b6e75]">{route.description}</p>
        </div>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#efefe9] px-2.5 py-1 text-xs font-bold text-[#44474e]"><MapPin size={13} aria-hidden="true" />{route.stops.length} POI</span>
      </div>

      {route.valid ? (
        <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#4d6410]"><CircleCheck size={14} aria-hidden="true" />Cấu hình tuyến hợp lệ</p>
      ) : (
      ...
      <div className={`grid gap-4 ${compact ? '' : 'lg:grid-cols-[minmax(0,1fr)_260px]'}`}>
        <ol className="space-y-2" aria-label={`Các điểm của ${route.name}`}>
          ...
          {route.stops.map((stop) => (
            <li key={stop.id} className="flex gap-3 rounded-xl border border-[#e3e3dc] bg-white px-3 py-2.5">
              <span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#1c1c1c] text-xs font-bold text-white tabular-nums" aria-hidden="true">{stop.order}</span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-[#1c1c1c]"><span className="sr-only">POI {stop.order}: </span>{stop.name}</p>
                <p className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-[#6b6e75]">
...
function Cap({ label, name, missing = false }: { label: string; name: string; missing?: boolean }) {
  return (
    <li className="flex items-center gap-3 px-3 py-1">
      <span className="grid size-7 shrink-0 place-items-center rounded-full border-2 border-[#dcdcd4] bg-white text-[#6b6e75]" aria-hidden="true"><Flag size={12} /></span>
      <p className="text-[13px] text-[#6b6e75]"><span className="font-bold text-[#3a3d44]">{label}:</span> <span className={missing ? 'font-bold text-[#b23e31]' : ''}>{name}</span></p>
    </li>
  )
}
...
function RouteSketch({ route }: { route: AdminRoute }) {
  ...
  return (
    <figure className="rounded-xl border border-[#e3e3dc] bg-[#f7f7f3] p-3">
      <svg viewBox={`0 0 ${maxX - minX} ${maxY - minY}`} className="aspect-[4/3] w-full" role="img" aria-label={`Sơ đồ vị trí các điểm của ${route.name}`}>
        <defs>
          <pattern id="route-grid" width="1" height="1" patternUnits="userSpaceOnUse"><path d="M1 0H0V1" fill="none" stroke="#e3e3dc" strokeWidth="0.04" /></pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#route-grid)" />
        {(() => {
          const s = at(route.startPoint.position)
          return <rect x={s.cx - 0.45} y={s.cy - 0.45} width="0.9" height="0.9" rx="0.2" fill="#fff" stroke="#6b6e75" strokeWidth="0.12" />
        })()}
        {route.stops.map((stop) => {
          const p = at(stop.position)
          return (
            <g key={stop.id}>
              <circle cx={p.cx} cy={p.cy} r="0.55" fill="#1c1c1c" />
              <text x={p.cx} y={p.cy + 0.2} textAnchor="middle" fontSize="0.6" fontWeight="700" fill="#fff">{stop.order}</text>
            </g>
          )
        })}
      </svg>
      <figcaption className="mt-2 text-[11px] leading-4 text-[#8e9096]">Vị trí trên bản đồ (■ xuất phát/kết thúc). Không vẽ đường nối vì robot tự tìm đường giữa các điểm.</figcaption>
    </figure>
  )
}
