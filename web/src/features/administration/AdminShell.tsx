...
import { Bot, LogOut, Menu, Radio, ShieldCheck, X } from 'lucide-react'
...
export default function AdminShell() {
  ...
  return (
    <div className="flex min-h-[100dvh] bg-[#f4f6fa] text-[#1f2937]">
      ...
      {menuOpen && <button type="button" onClick={closeMenu} className="fixed inset-0 z-30 cursor-default bg-[#1f2937]/25 backdrop-blur-[2px] lg:hidden" aria-label="Đóng điều hướng quản trị" />}

      <aside
        ref={navRef}
        tabIndex={-1}
        aria-label="Khu vực quản trị"
        className={`fixed top-0 left-0 z-40 flex h-[100dvh] w-64 flex-col border-r border-[#dfe4ec] bg-white transition-transform duration-200 lg:sticky lg:translate-x-0 focus-visible:outline-none ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex h-16 items-center border-b border-[#ecf0f5] px-4">
          <Link to="/admin" onClick={closeMenu} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#33415c]">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#33415c] text-white shadow-sm"><Bot size={20} aria-hidden="true" /></span>
          ...
          <button type="button" onClick={closeMenu} className="grid h-9 w-9 place-items-center rounded-xl text-[#8792a5] hover:bg-[#f4f6fa] lg:hidden" aria-label="Đóng menu"><X size={19} /></button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Điều hướng quản trị">
          ...
          <div className="mt-6 border-t border-[#ecf0f5] pt-5">
            <Link to="/staff" onClick={closeMenu} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-[#6b7688] transition-colors hover:bg-[#f4f6fa] hover:text-[#1f2937] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#33415c]">
      ...
      <div className="relative flex min-h-[100dvh] min-w-0 flex-1 flex-col overflow-hidden">
        <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center border-b border-[#dfe4ec] bg-white/90 px-5 backdrop-blur-md lg:px-8">
          <p className="text-sm font-bold tracking-[-0.01em] text-[#1f2937]">{ADMIN_NAV.find(({ path }) => active(path))?.label ?? 'Quản trị hệ thống'}</p>
        </header>
