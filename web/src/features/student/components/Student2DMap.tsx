    ...
    <div className="st-map">
      ...
      <div className="st-map__canvas">
        <svg viewBox="0 0 1000 700" preserveAspectRatio="xMidYMid slice" role="img" aria-label="Bản đồ khuôn viên với vị trí robot và các điểm tham quan">
          ...
          <path d="M 380 280 C 450 260, 560 300, 540 400 C 510 480, 420 460, 370 420 Z" fill={C.water} stroke="#17414c" strokeWidth="1.5" />
          <text x="455" y="370" fill="#5f98a6" fontSize="13" textAnchor="middle">Hồ Sen Sinh Thái</text>
          ...
          {/* Tuyến đường */}
          <polyline points={routePoints} fill="none" stroke={C.accent} strokeWidth="3" strokeDasharray="8 9" strokeLinecap="round" opacity="0.85" />
          ...
          {pois.map((poi) => {
            ...
            return (
              <g key={poi.id}>
                {isCurrent && <circle cx={px} cy={py} r="16" fill={C.accent} opacity="0.35" className="st-map__ping" />}
                <circle
                  cx={px}
                  cy={py}
                  r="15"
                  fill={isCurrent ? C.accent : isNext ? C.next : '#0b1519'}
                  stroke={isCurrent ? C.accent : isNext ? C.next : '#35505b'}
                  strokeWidth="2"
                />
                <text x={px} y={py + 4.5} fill={isCurrent || isNext ? '#0b1519' : C.white} fontSize="12" fontWeight="600" textAnchor="middle">
                  {poi.order}
                </text>
                <text x={px} y={py + 34} fill={isCurrent ? C.accent : '#c9d6db'} fontSize="13" fontWeight={isCurrent ? 600 : 400} textAnchor="middle">
                  {poi.title}
                </text>
              </g>
            )
          ...
          <g transform={`translate(${robotX}, ${robotY})`}>
            ...
            <g transform={`rotate(${robotPose.heading})`}>
              ...
              <line x1="0" y1="0" x2="0" y2="-36" stroke={C.accent} strokeWidth="2.5" strokeLinecap="round" />
            </g>
            <circle cx="0" cy="0" r="13" fill={C.white} stroke={C.accent} strokeWidth="3" />
            <circle cx="0" cy="0" r="4.5" fill="#0b1519" />
      ...
      <div className="st-map__foot">
        <div className="st-legend">
          <span><i style={{ background: '#34d3ef' }} />Điểm hiện tại</span>
          <span><i style={{ background: '#e7eef1' }} />Điểm kế tiếp</span>
          <span><i style={{ background: '#fff', boxShadow: '0 0 0 2px #34d3ef' }} />Robot AMR</span>
