  ...
  return (
    <StaffPage className="!bg-[#f7f7f3]">
      ...
      <section aria-label="Biểu đồ vận hành" className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <div className="rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)] p-5 shadow-xs">
      ...
      <div className="mt-6 grid items-start gap-5 min-[1200px]:grid-cols-[minmax(0,1.8fr)_minmax(300px,0.85fr)]">
        <section className="min-w-0" aria-label="Buổi đang chạy">
          ...
          {running ? (
            ...
            <div className="rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)] px-5 py-8 text-center shadow-xs">
              <p className="text-[14px] font-semibold text-[#6b6e75]">Hiện tại không có buổi nào đang chạy.</p>
              {next && (
      ...
      <div className="mt-6 grid items-start gap-5 min-[1200px]:grid-cols-[minmax(0,1.8fr)_minmax(300px,0.85fr)]">
        <section className="min-w-0" aria-label="Buổi hôm nay">
          ...
          />
          <ul className="divide-y divide-[#efefe9] rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)]">
            {tours.data.map((tour) => (
...
function KpiCard({
  ...
  return (
    <Link
      to={to}
      className="flex flex-col justify-between rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)] p-4 shadow-xs transition-all duration-200 hover:-translate-y-0.5 hover:border-[#c6c7cc] hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9cc93a]"
    >
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-bold text-[#6b6e75] truncate">{label}</span>
        <span
          className={`grid size-7 shrink-0 place-items-center rounded-lg ${
            tone === 'danger'
              ? 'bg-[#fef2f2] text-[#dc2626]'
              : tone === 'info'
              ? 'bg-[#f2f7e4] text-[#4d6410]'
              : 'bg-[#f7f7f3] text-[#6b6e75]'
          }`}
        >
      ...
      <div className="my-2 flex items-baseline gap-2">
        {live && <span className="size-2 rounded-full bg-[#8fbf2a] animate-pulse" />}
        <span
          className={`text-[26px] font-black tracking-tight tabular-nums leading-none ${
            tone ? TILE_TONE[tone] : 'text-[#1c1c1c]'
          }`}
        >
          {value}
        </span>
      ...
      {trend && (
        <span
          className={`text-[11px] font-semibold truncate ${
            trendTone ? TILE_TONE[trendTone] : 'text-[#8e9096]'
          }`}
        >
          {trend}
        </span>
  ...
  return (
    <div className="mb-2.5 flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
      <div className="flex min-w-0 items-baseline gap-2.5">
        <h2 className="text-[16px] font-bold tracking-tight text-[#1c1c1c]">{title}</h2>
        {note && <p className="truncate text-xs text-[#8e9096]">{note}</p>}
      </div>
      {action}
    </div>
  )
  ...
  return (
    <article
      className={`rounded-2xl border bg-white p-5 shadow-xs transition-colors duration-200 ${
        assist ? 'border-[#fca5a5] ring-1 ring-[#f87171]' : 'border-[#e3e3dc]'
      }`}
    >
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs font-bold text-[#4d6410] bg-[#f2f7e4] px-2 py-0.5 rounded-md">
              {tour.code}
            </span>
            <span className="size-1.5 rounded-full bg-[#8fbf2a]" />
            <span className="text-xs font-bold text-[#5f7a12]">Đang chạy</span>
          </div>
          <h3 className="mt-1 text-[18px] font-extrabold tracking-tight text-[#1c1c1c]">{tour.name}</h3>
...
function TourRow({ tour, now }: { tour: TourOperation; now: number }) {
  ...
  return (
    <li className="flex flex-wrap items-center gap-x-4 gap-y-2.5 px-5 py-3.5 transition-colors hover:bg-[#f7f7f3]">
      <span className="w-[84px] shrink-0">
        <span className="block text-[15px] font-extrabold text-[#1c1c1c] tabular-nums leading-none">
          {formatTime(tour.scheduledAt)}
        </span>
        {countdown && <span className="mt-1 block text-[11px] whitespace-nowrap text-[#8e9096]">{countdown}</span>}
      </span>
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-2">
          <span className="truncate text-sm font-bold text-[#1c1c1c]">{tour.name}</span>
          <span className="shrink-0 font-mono text-[11px] font-semibold text-[#6b6e75] bg-[#efefe9] px-1.5 py-0.2 rounded">
            {tour.code}
          </span>
        </span>
        <span className="mt-0.5 block truncate text-xs text-[#6b6e75]">
...
function AttentionRail({ items, now }: { items: AttentionItem[]; now: number }) {
  return (
    <section className="min-w-0" aria-label="Việc cần xử lý">
      ...
      {items.length === 0 ? (
        <div className="flex items-center gap-2.5 rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)] px-5 py-4 text-sm font-semibold text-[#5f7a12] shadow-xs">
          <CheckCheck size={18} aria-hidden="true" />
          Hệ thống hoạt động bình thường, không có sự cố.
        </div>
      ) : (
        <ul className="divide-y divide-[#efefe9] overflow-hidden rounded-2xl border border-[#ebebe4] bg-white shadow-[0_4px_20px_-8px_rgba(28,28,28,0.16)]">
          {items.map((item) => {
            const since = item.since ? formatElapsed(item.since, now) : null
            return (
              <li
                key={item.id}
                className={`border-l-4 p-4 transition-colors duration-200 hover:bg-[#f7f7f3] ${RAIL_EDGE[item.tone]}`}
              >
                <div className="flex gap-2.5">
                  <AlertTriangle size={16} className={`mt-0.5 shrink-0 ${RAIL_TEXT[item.tone]}`} aria-hidden="true" />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-bold text-[#1c1c1c]">{item.subject}</p>
                    <p className={`mt-0.5 text-xs font-semibold ${RAIL_TEXT[item.tone]}`}>{item.headline}</p>
                    {item.detail && (
                      <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-[#6b6e75]">{item.detail}</p>
                    )}
                    <div className="mt-2.5 flex items-center justify-between gap-2">
                      <span className="text-[11px] text-[#8e9096]">{since ?? ''}</span>
                      <Link
                        to={item.to}
