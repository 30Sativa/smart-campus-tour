...
import type { Assignment, MissionCommand } from '../../api/contracts/staff'
import { useAssignAmr, useMissionCommand, useReassignAmr, useStaffAmrs, useTourSession } from '../../features/staff/staff-hooks'
import { ErrorPanel, LoadingPanel, PageHeader, panelClass, StatusBadge } from '../../features/staff/StaffUi'
import { formatBattery, formatDateTime, formatTime } from '../../features/staff/formatters'
import { eventTypeLabel, statusLabel } from '../../features/staff/status'

/** Mission commands, in the words an operator reads. The key stays the API's. */
const COMMAND_LABEL: Record<MissionCommand, string> = {
  pause: 'tạm dừng',
  resume: 'tiếp tục',
  recall: 'gọi AMR về',
  cancel: 'hủy mission',
...
function AssignmentPanel({ sessionId, sessionStatus, assignments }: { sessionId: string; sessionStatus: string; assignments: Assignment[] }) {
  ...
  if (!isScheduled) return <p className="rounded-xl border border-[#dbe6f4] bg-[#f6f9fd] px-4 py-3 text-xs leading-5 text-[#647793]">Chỉ phiên tour chưa bắt đầu mới được gán hoặc gán lại AMR. Phiên hiện tại: <strong>{statusLabel(sessionStatus)}</strong>.</p>
  return <div className="space-y-4"><div className="space-y-2">{assignments.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl border border-[#edf2fa] px-4 py-3 text-sm"><div><p className="font-bold text-[#40546f]">{item.amrName}</p><p className="text-xs text-[#8a98ac]">Bởi {item.assignedBy || 'hệ thống'} · {formatDateTime(item.assignedAt)}</p></div><StatusBadge value={item.status} /></div>)}</div>
    {error && <p role="alert" className="rounded-xl border border-[#f5c8c2] bg-[#fff1ef] px-3 py-2 text-xs font-semibold text-[#c95042]">{error}</p>}
    <div className="grid gap-3 md:grid-cols-[1fr_auto]"><div className="grid gap-2"><label htmlFor={amrFieldId} className="text-xs font-bold text-[#516783]">AMR có sẵn</label><select id={amrFieldId} value={selected} onChange={(event) => setSelected(event.target.value)} className="min-h-11 rounded-xl border border-[#dce9fb] bg-white px-3 text-sm text-[#40546f] outline-none focus:border-[#6ba0ff]"><option value="">Chọn AMR…</option>{available.map((amr) => <option key={amr.id} value={amr.id}>{amr.name} · {statusLabel(amr.connectionState)} · {formatBattery(amr.batteryPercent)} · {statusLabel(amr.operationalState)}</option>)}</select><p className="text-[11px] text-[#8a98ac]">AMR phải đang chờ, còn trực tuyến, pin ít nhất 20% và chưa bận phiên khác.</p></div><div className="grid gap-2"><label htmlFor={reasonFieldId} className="text-xs font-bold text-[#516783]">Lý do {hasActive ? '(bắt buộc khi gán lại)' : '(tùy chọn)'}</label><input id={reasonFieldId} value={reason} onChange={(event) => setReason(event.target.value)} maxLength={1000} placeholder={hasActive ? 'Nhập lý do gán lại…' : 'Lý do gán AMR…'} className="min-h-11 rounded-xl border border-[#dce9fb] bg-white px-3 text-sm text-[#40546f] outline-none focus:border-[#6ba0ff]" /></div></div>
...
function MissionPanel({ sessionId, mission, sessionStatus }: { sessionId: string; mission?: { id: string; state: string; progressPercent: number } | null; sessionStatus: string }) {
  ...
  /**
   * The command confirmation is a real modal now. It was a fixed overlay with
   * no dialog role, no Escape and no focus move: by keyboard the page behind it
   * stayed reachable while the scrim said it was blocked, and a screen reader
  ...
  return <div className="space-y-4"><div className="rounded-xl border border-[#edf2fa] px-4 py-3"><div className="flex items-center justify-between gap-3"><StatusBadge value={state} /><span className="text-xs text-[#8a98ac]">Tiến độ {mission.progressPercent ?? 0}% · Phiên {sessionStatus}</span></div>{hint && <p role="status" className="mt-3 rounded-lg border border-[#cde9dc] bg-[#effbf5] px-3 py-2 text-xs font-semibold text-[#25895f]">{hint}</p>}{error && <p role="alert" className="mt-3 rounded-lg border border-[#f5c8c2] bg-[#fff1ef] px-3 py-2 text-xs font-semibold text-[#c95042]">{error}</p>}</div>
...
export default function SessionDetailPage() {
  ...
  if (detail.isPending) return <div className="min-h-full bg-[#eef2f8] p-5 lg:p-8"><LoadingPanel /></div>
  if (detail.isError) return <div className="min-h-full bg-[#eef2f8] p-5 lg:p-8"><ErrorPanel error={detail.error} onRetry={detail.refetch} /></div>
  if (!detail.data) return <div className="min-h-full bg-[#eef2f8] p-5 lg:p-8"><ErrorPanel error={new ApiError(404, 'Không tìm thấy phiên tour.')} /></div>
