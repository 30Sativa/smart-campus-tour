...
import { Activity, Bot, History, Radio } from 'lucide-react'
...
function MissionPanel({ sessionId, mission, sessionStatus }: { sessionId: string; mission?: { id: string; state: string; progressPercent: number } | null; sessionStatus: string }) {
  ...
  const execute = async (action: 'pause' | 'resume' | 'recall' | 'cancel' | 'emergency-stop') => {
    try { setPending(action); setHint(''); await command.mutateAsync({ sessionId, command: action, reason }); setConfirm(null); setReason(''); setError(''); setHint(action === 'emergency-stop' ? 'Đã ghi nhận lệnh E-Stop. Robot không được tự động tiếp tục từ thao tác khác.' : 'Đã gửi lệnh vận hành và ghi lại hành trình.') } catch (e) { setError(e instanceof ApiError ? e.body : 'Không thể gửi lệnh nhiệm vụ.') } finally { setPending(null) }
  }
  ...
  const canEmergency = state !== 'Completed' && state !== 'Cancelled'
  return <div className="space-y-4"><div className="rounded-xl border border-[#edf2fa] px-4 py-3"><div className="flex items-center justify-between gap-3"><StatusPill value={state} /><span className="text-xs text-[#8a98ac]">Tiến độ {mission.progressPercent ?? 0}% · Phiên {sessionStatus}</span></div>{hint && <p className="mt-3 rounded-lg border border-[#cde9dc] bg-[#effbf5] px-3 py-2 text-xs font-semibold text-[#25895f]">{hint}</p>}{error && <p className="mt-3 rounded-lg border border-[#f5c8c2] bg-[#fff1ef] px-3 py-2 text-xs font-semibold text-[#c95042]">{error}</p>}</div>
    ...
    <div className="rounded-xl border border-[#f5c8c2] bg-white p-4"><p className="text-xs font-bold text-[#c95042]">Khu vực ngắt khẩn cấp</p><p className="mt-1 text-xs leading-5 text-[#71819a]">E-Stop yêu cầu xác nhận riêng và không bao giờ làm robot tự động tiếp tục hoạt động.</p><button type="button" disabled={!canEmergency || pending !== null} onClick={() => setConfirm('emergency-stop')} className="mt-3 inline-flex min-h-11 items-center justify-center rounded-xl bg-[#c95042] px-4 text-sm font-bold text-white hover:bg-[#b33e30] disabled:opacity-50">Ngắt khẩn cấp</button></div>
    {confirm && <div className="fixed inset-0 z-40 flex items-center justify-center bg-[#1f314d]/30 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-2xl border border-[#dce9fb] bg-white p-5 shadow-[0_18px_60px_rgba(31,49,77,0.22)]"><h4 className="text-base font-bold text-[#1f314d]">Xác nhận lệnh {confirm}</h4><p className="mt-1 text-sm leading-6 text-[#71819a]">Thao tác này sẽ được kiểm tra trạng thái ở máy chủ và ghi vào nhật ký kiểm toán{confirm === 'emergency-stop' ? '. E-Stop không được tự động tiếp tục.' : '.'}</p><label className="mt-4 grid gap-2 text-xs font-bold text-[#516783]">Lý do / ghi chú<textarea value={reason} onChange={(event) => setReason(event.target.value)} maxLength={1000} rows={3} placeholder={confirm === 'emergency-stop' ? 'Lý do an toàn bắt buộc…' : 'Lý do thao tác…'} className="rounded-xl border border-[#dce9fb] px-3 py-2 text-sm font-normal text-[#40546f] outline-none focus:border-[#6ba0ff]" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => { setConfirm(null); setReason('') }} className="min-h-10 rounded-xl border border-[#dce9fb] bg-white px-4 text-sm font-bold text-[#647793]">Hủy</button><button type="button" onClick={() => execute(confirm as 'pause' | 'resume' | 'recall' | 'cancel' | 'emergency-stop')} disabled={pending !== null} className="min-h-10 rounded-xl bg-[#1f314d] px-4 text-sm font-bold text-white hover:bg-[#2f4768] disabled:opacity-60">{pending ? 'Đang gửi…' : 'Xác nhận'}</button></div></div></div>}
  </div>
}
...
export default function StaffSessionDetailPage() {
  ...
  return <div className="min-h-full bg-[#f1f6fe] px-4 py-5 sm:px-6 lg:px-8 lg:py-7"><div className="mx-auto w-full max-w-[1500px]">
    <PageHeader icon={<Activity size={14} />} title={session.routeName} description={`Phiên ${session.id.slice(0, 8)} · ${formatDateTime(session.startTime)} đến ${formatTime(session.endTime)} · Booking ${session.bookingId.slice(0, 8)}`} />
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
      <div className="space-y-5">
        <section className={panelClass}><div className="border-b border-[#edf2fa] px-5 py-4"><h3 className="font-bold text-[#40546f]">Thông tin phiên tour</h3></div><div className="grid gap-3 p-5 text-sm"><div className="flex items-center justify-between"><span className="text-[#71819a]">Trạng thái phiên</span><StatusPill value={session.status} /></div><div className="flex items-center justify-between"><span className="text-[#71819a]">Khách tham quan</span><span className="font-semibold text-[#40546f]">{session.visitorName}</span></div><div className="flex items-center justify-between"><span className="text-[#71819a]">AMR hiện tại</span><span className="font-semibold text-[#40546f]">{session.amrName || 'Chưa gán'}</span></div><div className="flex items-center justify-between"><span className="text-[#71819a]">Mission</span><StatusPill value={session.mission?.state || 'Không có'} /></div></div></section>
